...
//! fast tier physically held objects the stack believed were in PMEM. The unit
//! tests could not see it -- they drive the stack directly and never place any
//! bytes -- and `set_places_a_brand_new_key_directly_in_slow_once_admission_is_
//! latched` below, which catches it immediately, existed only for the baseline.
//!
...
//! admission checks fast-tier capacity directly (see `LfuCompactHybridStack`'s
...
mod hybrid_cache_tests {
    ...
    fn ttl_survives_a_demotion() {
        ...
        let cache = PaperCache::<u32, TieredBuffer>::new(
            1_048_576,
            CacheTierSize::Bytes(TTL_FAST_TIER), PaperPolicy::LfuCompactHybrid).expect("cache should construct");
    ...
    fn fast_tier_value_pressure_routes_to_slow_without_evicting() {
        ...
        // per-process binary tests/lfu_compact_hybrid_cache_shared_overhead.rs.
        let cache = PaperCache::<u32, TieredBuffer>::new(
            1_048_576,
            CacheTierSize::Bytes(2_048), PaperPolicy::LfuCompactHybrid).expect("cache should construct");
    ...
    fn set_places_a_brand_new_key_directly_in_slow_once_admission_is_latched() {
        ...
        // `LfuCompactHybridStack`'s module doc on the admission latch).
        let cache = PaperCache::<u32, TieredBuffer>::new(
            1_048_576,
            CacheTierSize::Bytes(DEMOTES_ONE_OF_TWO), PaperPolicy::LfuCompactHybrid).expect("cache should construct");
