...
//! that class of defect, and `lru-lfu`'s admission rule -- which has to look up
//! an EXISTING key's tier rather than answer from "is this key new" -- is
//! exactly the kind that a missing arm silently replaces with something
//! plausible.
//!
//! Run with nightly (required for `allocator_api` via `key_value_pmem`):
//!   cargo +nightly test --test lru_lfu_compact_hybrid_cache_integration --features lru_lfu_compact_hybrid_cache
//!
...
mod hybrid_cache_tests {
    ...
    fn ensure_pmem_allocator_warm() {
        ...
        let cache = PaperCache::<u32, TieredBuffer>::new(1_048_576, CacheTierSize::Bytes(1), PaperPolicy::LruLfuCompactHybrid(2))
    ...
    fn make_cache() -> PaperCache<u32, TieredBuffer> {
        PaperCache::<u32, TieredBuffer>::new(
            MAX_SIZE,
            CacheTierSize::Bytes(FAST_TIER), PaperPolicy::LruLfuCompactHybrid(PROMOTE_K)).expect("cache should construct")
    }
    ...
    fn admission_always_lands_in_the_fast_tier() {
        let cache = PaperCache::<u32, TieredBuffer>::new(
            MAX_SIZE,
            CacheTierSize::Bytes(MAX_SIZE), PaperPolicy::LruLfuCompactHybrid(PROMOTE_K)).expect("cache should construct");
    ...
    fn ttl_survives_a_demotion() {
        ...
        let cache = PaperCache::<u32, TieredBuffer>::new(
            MAX_SIZE,
            CacheTierSize::Bytes(4_500), PaperPolicy::LruLfuCompactHybrid(PROMOTE_K)).expect("cache should construct");
    ...
    fn invalid_fast_tier_sizes_are_rejected() {
        let zero = PaperCache::<u32, TieredBuffer>::new(
            MAX_SIZE,
            CacheTierSize::Bytes(0), PaperPolicy::LruLfuCompactHybrid(PROMOTE_K));
        assert!(matches!(zero, Err(CacheError::InvalidFastTierSize)));

        let too_big = PaperCache::<u32, TieredBuffer>::new(
            MAX_SIZE,
            CacheTierSize::Bytes(MAX_SIZE + 1), PaperPolicy::LruLfuCompactHybrid(PROMOTE_K));
    ...
    fn a_zero_promotion_threshold_is_rejected() {
        ...
        let bad = PaperCache::<u32, TieredBuffer>::new(
            MAX_SIZE,
            CacheTierSize::Bytes(FAST_TIER), PaperPolicy::LruLfuCompactHybrid(0));
