...
mod tiering_tests {
    ...
    fn test_tiering_manager_integration() {
        ...
        println!(
            "Promotions: {}, DRAM objects: {}",
            stats.promotions, stats.dram_objects
        );
    }
...
mod tiering_pmem_key_tests {
    ...
    fn test_new_objects_start_in_pmem_only() {
        ...
        let stats = cache.tiering_stats();
        assert_eq!(
            stats.pmem_only_objects, 5,
            "all objects should start PMEM-only"
        );
    ...
    fn test_hot_object_promoted_to_dram() {
        ...
        assert!(
            ...
            "expected object in DRAM after promotion"
        );
    }
    ...
    fn test_dram_copy_data_integrity() {
        ...
        assert_eq!(
            result, expected,
            "data integrity check failed after promotion"
        );
    }
    ...
    fn test_wipe_resets_tiering_state() {
        ...
        let stats = cache.tiering_stats();
        assert_eq!(
            stats.pmem_only_objects, 0,
            "pmem_only_objects should be 0 after wipe"
        );
...
mod hashtable_tiering_tests {
    use paper_cache::{BufferPMEM, PaperCache, PaperPolicy, TieringConfig};
    use std::thread;
    use std::time::Duration;
    ...
    fn test_hashtable_tiering_warm_and_hot_tiers() {
        ...
        let stats = cache.tiering_stats();
        println!(
            "After 2 accesses - DRAM objects: {}, Promotions: {}, PMEM only: {}",
        ...
        assert!(
            stats.dram_objects >= 1,
            "Expected at least 1 object in DRAM (warm tier)"
        );
        ...
        let stats = cache.tiering_stats();
        println!(
            "After 5 accesses - DRAM objects: {}, Promotions: {}, DRAM size: {}",
        ...
        assert!(
            stats.dram_objects >= 1,
            "Expected at least 1 object in DRAM (hot tier)"
        );
        assert!(
            stats.promotions >= 2,
            "Expected at least 2 promotions (warm + hot)"
        );
        // Hot tier means physical copy, so dram_size should be > 0
        assert!(
            stats.dram_size > 0,
            "Expected DRAM size > 0 for hot tier physical copy"
        );
    ...
    fn test_hashtable_tiering_pointer_tier_zero_copy() {
        ...
        let stats = cache.tiering_stats();
        println!(
            "Pointer tier test - DRAM objects: {}, DRAM size: {}",
        ...
        assert!(
            stats.dram_objects >= 1,
            "Expected object in DRAM (warm tier)"
        );
...
mod multitiering_tests {
    ...
    fn test_multitiering_warm_tier_promotion() {
        ...
        assert!(
            stats.dram_objects >= 1,
            "expected object promoted to warm tier"
        );
        assert!(stats.promotions >= 1, "expected at least one promotion");
    }
    ...
    fn test_multitiering_hot_tier_promotion() {
        ...
        assert!(
            stats.dram_objects >= 1,
            "expected object in DRAM (hot tier)"
        );
        assert!(stats.promotions >= 2, "expected warm + hot promotions");
        assert!(
            stats.dram_size > 0,
            "hot tier should have non-zero DRAM size"
        );
    }
    ...
    fn test_multitiering_data_integrity_after_hot_promotion() {
        ...
        assert_eq!(
            result, expected,
            "data integrity check failed after hot promotion"
        );
    }
...
mod hybridcache_promotion_tests {
    ...
    fn test_ghost_hit_promotion_only() {
        ...
        assert!(
            ...
            stats.promotions, stats.dropped_promotions
        );
        ...
        let _ = cache
            .get(&99u32)
            .expect("pmem get for non-ghost key failed");
        let non_ghost_promoted =
            wait_until(Duration::from_millis(500), || cache.has_in_dram(&99u32));
        assert!(
            !non_ghost_promoted,
