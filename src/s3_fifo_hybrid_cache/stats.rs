...
//! Statistics for `s3_fifo_hybrid_cache`.
//!
...
/// A point-in-time snapshot of `s3_fifo_hybrid_cache` statistics.
///
/// Assembled by `AtomicStatus::s3_fifo_hybrid_stats` and returned from
/// `PaperCache::s3_fifo_hybrid_stats`.
#[derive(Debug, Clone, Default, PartialEq, Eq)]
pub struct S3FifoHybridStats {
	/// Objects moved from the one-access queue (eager, on re-access) or
	/// given a second chance out of the main queue's tail (lazy, at
	/// eviction time) into the main queue's fast portion.
	pub promotions: u64,
