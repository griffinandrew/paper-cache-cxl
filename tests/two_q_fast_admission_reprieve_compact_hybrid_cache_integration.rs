...
//! happens to do.
//!
//! Exactly one test departs from the rename --
//! `global_capacity_pressure_still_evicts_and_loses_nothing` -- and only in the
//! number of bytes it writes; see the comment on it.
//!
//! Run with nightly (required for `allocator_api` via `key_value_pmem`):
//!   cargo +nightly test --test two_q_fast_admission_reprieve_compact_hybrid_cache_integration --features two_q_fast_admission_reprieve_compact_hybrid_cache
//!
...
//!
//! What is tested:
//!   * Admission still lands in the fast tier immediately, with no migration
//!   * An aged-out one-access key survives in the slow tier, is still
//!     readable, and is counted as a demotion rather than an eviction
//!   * A reprieved key is a full main-queue citizen: re-accessing it promotes
//!     it back to fast
//!   * Terminal eviction still fires under real global capacity pressure and
//!     loses nothing
//!   * Main-queue fast-tier pressure still demotes normally, and total fast
//!     usage stays within the configured budget
//!   * TTL set before a tier move is still correctly enforced after
//!   * `set_fast_tier_size`/`resize` take effect at runtime
//!   * `hybrid_stats()` reports the tier movement that occurred
//!   * Zero/invalid fast-tier-size and `k_in` edge cases

#[cfg(feature = "two_q_fast_admission_reprieve_compact_hybrid_cache")]
mod hybrid_cache_tests {
    ...
    //
    // 256 puts ten objects at 2_560, unambiguously over: the settle drains to
    // the ~781-byte budget, leaving two objects fast and eight slow. The FIFO
    ...
    // included, but NOT applied file-wide: `many_admissions_all_land_fast_without_any_migration` asserts
    ...
    fn make_cache() -> PaperCache<u32, TieredBuffer> {
        PaperCache::<u32, TieredBuffer>::new(
            MAX_SIZE,
            CacheTierSize::Bytes(FAST_TIER), PaperPolicy::TwoQFastAdmissionReprieveCompactHybrid(K_IN)).expect("cache should construct")
    }
    ...
    fn ensure_pmem_allocator_warm() {
        ...
        let cache = PaperCache::<u32, TieredBuffer>::new(
            1_048_576,
            CacheTierSize::Bytes(400), PaperPolicy::TwoQFastAdmissionReprieveCompactHybrid(0.1)).expect("warm-up cache should construct");
    ...
    fn admission_lands_in_the_fast_tier_immediately() {
        ...
        let cache = PaperCache::<u32, TieredBuffer>::new(
            1_048_576,
            CacheTierSize::Bytes(524_288), PaperPolicy::TwoQFastAdmissionReprieveCompactHybrid(0.1)).expect("cache should construct");
    ...
    fn many_admissions_all_land_fast_without_any_migration() {
        ...
        let cache = PaperCache::<u32, TieredBuffer>::new(
            1_048_576,
            CacheTierSize::Bytes(524_288), PaperPolicy::TwoQFastAdmissionReprieveCompactHybrid(0.5)).expect("cache should construct");
    ...
    fn fast_tier_usage_stays_within_the_configured_budget() {
        ...
        let cache = PaperCache::<u32, TieredBuffer>::new(
            1_048_576,
            CacheTierSize::Bytes(FAST_TIER_BYTES), PaperPolicy::TwoQFastAdmissionReprieveCompactHybrid(0.001)).expect("cache should construct");
    ...
    /// The defining behavior: FIFO-capacity pressure must produce
    /// *demotions*, not evictions, and must not lose any object.
    #[test]
    fn fifo_capacity_pressure_reprieves_instead_of_evicting() {
        ensure_pmem_allocator_warm();

        // Tiny k_in so the FIFO queue overflows immediately and reprieves
        // continuously; max_size large enough that global capacity eviction
        // never fires, isolating the reprieve path.
        let cache = PaperCache::<u32, TieredBuffer>::new(
            10_485_760,
            CacheTierSize::Bytes(524_288), PaperPolicy::TwoQFastAdmissionReprieveCompactHybrid(0.0001)).expect("cache should construct");
        ...
        assert!(
            ...
            "FIFO pressure should have reprieved (demoted), not evicted",
        );
    ...
    #[test]
    fn a_reprieved_key_ends_up_in_the_slow_tier_and_reads_back_correctly() {
        ensure_pmem_allocator_warm();
        ...
        assert!(
            ...
            "an aged-out one-access key should be in the slow tier",
        );

        let slow_key = slow_key.expect("checked above");

        // The bytes physically moved to PMEM and still read back intact.
        assert_eq!(cache.get(&slow_key).unwrap(), vec![slow_key as u8; 64]);
    }

    /// A reprieved key is a full main-queue citizen: a later access promotes
    /// it back to DRAM like any other slow key.
    #[test]
    fn a_reprieved_key_can_still_be_promoted_by_a_later_access() {
        ...
        assert!(
            ...
            "should have a reprieved key to promote",
        );

        let slow_key = slow_key.expect("checked above");

        cache.get(&slow_key).expect("get on a reprieved key should hit");

        assert!(
            wait_until(MIGRATION_TIMEOUT, || {
                cache.tier_of(&slow_key) == Some(Tier::Fast)
            }),
            "re-accessing reprieved key {slow_key} should promote it back to fast",
        );

        assert!(cache.hybrid_stats().promotions > 0);
    }

    /// Terminal eviction still fires under real global capacity pressure --
    /// a reprieve defers eviction, it does not abolish it.
    ///
    /// THE ONE DELIBERATE DEPARTURE from the baseline suite, and it is about
    /// the test's premise rather than about behaviour. The cache charges
    /// `base_size + get_policy_overhead(policy)` per object, and this policy's
    /// overhead term is 16 + 24 = 40 where the baseline's is
    /// (48 + 8) + (24 + 1 + 1 + 4) = 86 -- the gap is the conversion's whole
    /// point. Measured through `cache.size()`, the baseline charges 170
    /// B/object here and this stack charges 124, so the baseline's 60 x 64-byte
    /// objects came to 10,200 bytes against an 8,192-byte budget and this
    /// stack's came to 7,440: over budget in one case, comfortably under it in
    /// the other. The test did not fail because eviction diverged; it failed
    /// because there was no longer any global capacity pressure to observe.
    ///
    /// 256-byte values overflow the budget on payload alone (60 x 256 =
    /// 15,360), which restores the premise without retuning the numbers to
    /// this policy's overhead -- any policy overflows here.
    #[test]
    fn global_capacity_pressure_still_evicts_and_loses_nothing() {
        ensure_pmem_allocator_warm();

        let cache = PaperCache::<u32, TieredBuffer>::new(
            8_192,
            CacheTierSize::Bytes(4_096), PaperPolicy::TwoQFastAdmissionReprieveCompactHybrid(0.05)).expect("cache should construct");
        ...
        assert!(
            ...
            "global capacity pressure should still evict",
        );

        // Objects still present should equal admissions minus evictions --
        // i.e. reprieves did not silently lose or double-count anything.
        assert!(
            wait_until(MIGRATION_TIMEOUT, || {
                let stats = cache.hybrid_stats();
                let present = (1..=60u32).filter(|key| cache.has(key)).count() as u64;
                present == 60 - stats.evictions
            }),
            "present objects should equal admissions minus evictions",
        );
    }
    ...
    fn set_fast_tier_size_takes_effect_at_runtime() {
        ...
        let cache = PaperCache::<u32, TieredBuffer>::new(
            1_048_576,
            CacheTierSize::Bytes(524_288), PaperPolicy::TwoQFastAdmissionReprieveCompactHybrid(0.001)).expect("cache should construct");
    ...
    fn resize_rescales_the_fifo_reservation_and_re_settles() {
        ...
        let cache = PaperCache::<u32, TieredBuffer>::new(
            2_048,
            CacheTierSize::Bytes(1_500), PaperPolicy::TwoQFastAdmissionReprieveCompactHybrid(0.5)).expect("cache should construct");
    ...
    fn invalid_construction_parameters_are_rejected() {
        // Zero fast tier.
        assert!(matches!(
            PaperCache::<u32, TieredBuffer>::new(1_048_576, CacheTierSize::Bytes(0), PaperPolicy::TwoQFastAdmissionReprieveCompactHybrid(0.1)),
            Err(CacheError::InvalidFastTierSize),
        ));

        // Fast tier larger than the whole cache.
        assert!(matches!(
            PaperCache::<u32, TieredBuffer>::new(1_024, CacheTierSize::Bytes(2_048), PaperPolicy::TwoQFastAdmissionReprieveCompactHybrid(0.1)),
            Err(CacheError::InvalidFastTierSize),
        ));

        // Zero cache size.
        assert!(matches!(
            PaperCache::<u32, TieredBuffer>::new(0, CacheTierSize::Bytes(1), PaperPolicy::TwoQFastAdmissionReprieveCompactHybrid(0.1)),
            Err(CacheError::ZeroCacheSize),
        ));

        // k_in outside [0.0, 1.0].
        assert!(matches!(
            PaperCache::<u32, TieredBuffer>::new(1_048_576, CacheTierSize::Bytes(1_024), PaperPolicy::TwoQFastAdmissionReprieveCompactHybrid(1.5)),
            Err(CacheError::InvalidPolicy),
        ));

        assert!(matches!(
            PaperCache::<u32, TieredBuffer>::new(1_048_576, CacheTierSize::Bytes(1_024), PaperPolicy::TwoQFastAdmissionReprieveCompactHybrid(-0.1)),
            Err(CacheError::InvalidPolicy),
        ));
    }
    ...
    fn a_fifo_reservation_covering_the_whole_fast_tier_still_works() {
        ...
        let cache = PaperCache::<u32, TieredBuffer>::new(
            10_240,
            CacheTierSize::Bytes(1_024), PaperPolicy::TwoQFastAdmissionReprieveCompactHybrid(1.0)).expect("cache should construct");
