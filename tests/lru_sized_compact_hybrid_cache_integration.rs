...
//! Integration tests for the `lru_sized_compact_hybrid_cache` feature.
//!
...
mod hybrid_cache_tests {
    ...
    /// IGNORED for the compact stack: the scenario is not reachable here, and
    /// that is a property of the overhead accounting rather than of this stack.
    ///
    /// It requires being over `max_size` while BOTH slow lists are empty and
    /// nothing has ever demoted. Two different per-object charges decide that:
    /// `get_policy_overhead`, counted against `max_size`, and
    ...
    /// before the reservation squeezes the segments into demoting. Compaction
    /// cut the first by 53% and the second by only 17%, so the segments now
    /// run out relatively earlier and the window closes. Roughly 200
    /// configurations were swept -- object counts 10-40, capacities 512-3019,
    /// and cap/max ratios from 60% to 100%, including the baseline\'s own
    /// 89% shape scaled by the measured used_size ratio of 760/1180 -- and
    /// NONE produced an eviction with zero demotions.
    ///
    /// This is the same unreconciled pair of overhead models tracked
    /// elsewhere: `get_policy_overhead` is a hand-itemised table (the compact
    /// entries are a flat `16 + 24` written by the registration helper and
    /// never measured), while `get_hybrid_dram_shared_overhead` is built from
    /// the measured eviction-stack constants. Reconciling them is what would
    /// make this test meaningful again; forcing a configuration that happens
    /// to pass would not.
    ///
    /// The baseline suite still runs this test unignored, so the path itself
    /// stays covered.
    #[test]
    #[ignore]
    fn terminal_eviction_falls_back_to_the_more_over_budget_fast_segment_when_slow_is_empty() {
        ...
        // a broken one. 760 was measured the same way the baseline's 1180
        // was; the original margin of 30 bytes is kept.
        //
        // For the record, the baseline's own note: with 10 tiny ("payload
        // bytes", 13-byte) objects and equal 1000-byte small/large
        // capacities, real `status().used_size()` settles at 1180 (base bytes plus the
    ...
    fn small_segment_value_pressure_demotes_without_evicting() {
        ...
        // per-process binary tests/lru_sized_compact_hybrid_cache_shared_overhead.rs.
        let cache = PaperCache::<u32, TieredBuffer>::new_sized_compact(
            1_048_576,
    ...
    fn zero_small_fast_tier_size_is_rejected() {
        let result = PaperCache::<u32, TieredBuffer>::new_sized_compact(
