...
/// Marker type selecting `s3_fifo_hybrid_cache`'s behavior for the shared
/// generic `impl<K, S> PaperCache<K, TieredBuffer, S>` block in `lib.rs`
/// (see `crate::hybrid_policy::HybridPolicy`). Admission is unconditional to
/// the *slow* tier: every new object starts in the one-access FIFO queue
/// (`S3FifoHybridStack::insert`); only a re-access promotes it — eagerly if
/// still in the one-access queue, lazily (at eviction time) if already in
/// the main queue. `ExtraConfig = f64` carries the one-access queue's byte
/// budget as a fraction of `max_size`, same shape as `TwoQHybridPolicy`'s
/// `k_in`.
pub struct S3FifoHybridPolicy;

impl crate::hybrid_policy::HybridPolicy for S3FifoHybridPolicy {
	type Stats = S3FifoHybridStats;
	type ExtraConfig = f64;

	fn seed_policy(ratio: f64) -> crate::PaperPolicy {
		crate::PaperPolicy::S3FifoHybrid(ratio)
	}
