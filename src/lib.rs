...
#![cfg_attr(any(feature = "key_value_pmem", feature = "global_hashtable_pmem", feature = "tiering_hashtable_pmem", feature = "flatmap_dram", feature = "flatmap_pmem", feature = "global_flatmap_dram", feature = "global_flatmap_pmem", feature = "eviction_stacks_pmem", feature = "admission_control"), feature(allocator_api))]
...
#[cfg(any(feature = "key_value_pmem", feature = "global_hashtable_pmem", feature = "tiering_hashtable_pmem", feature = "flatmap_pmem", feature = "global_flatmap_pmem", feature = "eviction_stacks_pmem", feature = "admission_control"))]
pub mod allocator;
...
#[cfg(all(feature = "key_value_pmem", any(feature = "enable_tiering_manager", feature = "sets_dram", feature = "admission_control")))]
pub mod tiering;
    ...
    S: Default + Clone + BuildHasher,
{
	...
	pub fn with_hasher(
		...
		#[cfg(all(feature = "key_value_pmem", feature = "enable_tiering_manager", not(feature = "sets_dram"), not(feature = "admission_control")))]
		let tiering_manager = {
			// Create tiering manager with default DRAM threshold at 20% of max_size
			let tiering_config = tiering::TieringConfig::default();
			Arc::new(TieringManager::new(tiering_config))
		...
		#[cfg(all(feature = "key_value_pmem", feature = "sets_dram", not(feature = "admission_control")))]
		let tiering_manager = {
			let tiering_config = tiering::TieringConfig::default();
		...
		// admission_control: DRAM-first (4 GB limit via HybridObjects) with CXL victim cache
		#[cfg(all(feature = "key_value_pmem", feature = "admission_control"))]
		let tiering_manager = {
			let mut tiering_config = tiering::TieringConfig::default();
			// Set hotness threshold to 3 accesses for the 2Q lifecycle
			tiering_config.hotness_threshold = 3;
			Arc::new(TieringManager::new_admission_control(tiering_config))
		};

		let (worker_sender, worker_listener) = unbounded();

		#[cfg(all(feature = "enable_tiering_manager", not(feature = "sets_dram"), not(feature = "admission_control")))]
		let mut worker_manager = WorkerManager::new(
			worker_listener,
			&objects,
			&status,
		...
		#[cfg(all(feature = "sets_dram", not(feature = "admission_control")))]
		let mut worker_manager = WorkerManager::new(
			worker_listener,
			&objects,
			&status,
		...
		#[cfg(all(not(feature = "enable_tiering_manager"), not(feature = "sets_dram"), not(feature = "admission_control")))]
		let mut worker_manager = WorkerManager::new(
			worker_listener,
			&objects,
			&status,
		...
		// admission_control uses the basic WorkerManager (no tiering in the policy worker)
...
/*
	...
	*/
	...
	pub fn get(&self, key: &K) -> Result<Vec<u8>, CacheError>
	{
		...
		// admission_control: check CXL victim cache for items previously evicted from DRAM.
		// get_from_victim records the access and handles Victim→Warm transitions inline.
		// If access count reaches hotness_threshold, promote the object back to DRAM.
		#[cfg(all(feature = "admission_control", not(feature = "enable_tiering_manager")))]
		if let Some(victim_bytes) = self.tiering_manager.get_from_victim(hashed_key, key) {
			self.status.incr_hits();
			// Record access; if threshold reached, promote from victim cache into DRAM.
			// NOTE: A small TOCTOU window exists between get_from_victim and the insert.
			// Concurrent deletes during this window are an accepted trade-off.
			if self.tiering_manager.record_access(hashed_key) {
				// Warm→Hot: re-insert into DRAM objects map via Hybrid (DRAM when space available)
				let key_owned = key.clone();
				let mut dram_vec = Vec::<u8, Hybrid>::with_capacity_in(victim_bytes.len(), Hybrid);
				dram_vec.extend_from_slice(&victim_bytes);
				let new_buf: BufferPMEM = dram_vec.into_boxed_slice();
				let new_obj = Object::new(key_owned, new_buf, None);
				let base_size = self.overhead_manager.base_size(&new_obj);
				self.objects.insert(hashed_key, new_obj);
				self.status.update_base_used_size(base_size as i64);
				self.tiering_manager.mark_hot(hashed_key);
			}
			self.broadcast(WorkerEvent::Get(hashed_key, true))?;
			return Ok(victim_bytes);
		}
		...
		// admission_control 2Q lifecycle: record access, trigger synchronous Warm→Hot promotion
		// for objects still in the main objects map (not yet evicted to CXL).
		// NOTE: A small TOCTOU window exists between dropping the read guard and re-inserting.
		#[cfg(all(feature = "admission_control", not(feature = "enable_tiering_manager")))]
		if result.is_ok() {
			if self.tiering_manager.record_access(hashed_key) {
				// Promote from Warm to Hot: re-allocate via Hybrid (DRAM when space is free)
				if let Some(obj) = self.objects.get(&hashed_key) {
					let bytes: Vec<u8> = obj.data().as_ref().to_vec();
					let expiry = obj.expiry();
					let key_owned: K = obj.value().key().clone();
					drop(obj); // release DashMap read guard before writing
					let mut dram_vec = Vec::<u8, Hybrid>::with_capacity_in(bytes.len(), Hybrid);
					dram_vec.extend_from_slice(&bytes);
					let new_buf: BufferPMEM = dram_vec.into_boxed_slice();
					let new_obj = Object::with_expiry(key_owned, new_buf, expiry);
					self.objects.insert(hashed_key, new_obj);
					self.tiering_manager.mark_hot(hashed_key);
				}
			}
		...
		K: 'static + Eq + Hash + TypeSize + std::fmt::Debug,
	{
		...
		// admission_control: DRAM-first via Hybrid (4 GB limit, CXL overflow when full)
		#[cfg(all(feature = "admission_control", not(feature = "sets_dram")))]
		{
			let mut val_vec = Vec::<u8, Hybrid>::with_capacity_in(value.len(), Hybrid);
			val_vec.extend_from_slice(value);
			let val_buf: BufferPMEM = val_vec.into_boxed_slice();
			let object = Object::new(key.clone(), val_buf, ttl);
			let base_size = self.overhead_manager.base_size(&object);
			let expiry = object.expiry();
			if base_size == 0 { return Err(CacheError::ZeroValueSize); }
			if self.status.exceeds_max_size(base_size) { return Err(CacheError::ExceedingValueSize); }
			self.status.incr_sets();
			let old_object_info = self.objects.insert(hashed_key, object)
				.map(|old| {
					let old_size = self.overhead_manager.base_size(&old);
					let old_expiry = old.expiry();
					(old_size, old_expiry)
				});
			...
			self.status.update_base_used_size(base_size_delta);
			// Register with TieringManager as Admission state (newly admitted to DRAM)
			self.tiering_manager.register_admission(hashed_key, base_size as u32);
			self.broadcast(WorkerEvent::Set(hashed_key, base_size, expiry, old_object_info))?;
			return Ok(());
		}
