...
// UMF allocator bindings are only needed when we might call into PMEM hardware.
// When `all_dram` is enabled, HybridObjects falls back to jemalloc and never
// calls UMF, so we must not include the extern "C" declarations (which would
// cause linker errors on machines without the UMF shared library).
#[cfg(all(
    any(feature = "key_value_pmem", feature = "global_hashtable_pmem", feature = "tiering_hashtable_pmem", feature = "flatmap_pmem", feature = "global_flatmap_pmem", feature = "eviction_stacks_pmem"),
