...
compile_error!(
    "Cannot enable both 'global_flatmap_dram' and 'global_flatmap_pmem' features simultaneously. Please choose only one FlatMap mode for the global hashtable."
);
...
compile_error!(
    "Cannot enable both 'hashbrown_dram' and 'global_hashtable_pmem' features simultaneously. Please choose only one global hashtable mode."
);

#[cfg(all(feature = "hashbrown_dram", feature = "global_flatmap_dram"))]
compile_error!(
    "Cannot enable both 'hashbrown_dram' and 'global_flatmap_dram' features simultaneously. Please choose only one global hashtable mode."
);

#[cfg(all(feature = "hashbrown_dram", feature = "global_flatmap_pmem"))]
compile_error!(
    "Cannot enable both 'hashbrown_dram' and 'global_flatmap_pmem' features simultaneously. Please choose only one global hashtable mode."
);
...
#[cfg(any(
    all(feature = "key_value_pmem", feature = "enable_tiering_manager"),
    all(feature = "key_value_pmem", feature = "sets_dram")
))]
pub mod tiering;
...
#[cfg(feature = "hw_perf")]
pub use crate::hw_perf_counters::{
    HwHashMapStats, HwPerfMeasurement, get_hw_counters, get_hw_hashmap_stats, measure_operation,
    print_hw_perf_stats,
};
...
pub use crate::tiering::{TieringConfig, TieringManager, TieringStats};
...
#[cfg(feature = "key_value_pmem")]
impl From<&[u8]> for BufferPMEM {
    fn from(value: &[u8]) -> Self {
        value.to_vec_in(Hybrid).into_boxed_slice()
    }
}

#[cfg(feature = "key_value_pmem")]
impl From<Vec<u8>> for BufferPMEM {
    fn from(value: Vec<u8>) -> Self {
        let mut vec = Vec::with_capacity_in(value.len(), Hybrid);
...
#[cfg(all(
    ...
    not(feature = "global_flatmap_pmem"),
...
pub type ObjectMapRef<K, V> = Arc<
    RwLock<HashMap<HashedKey, Object<K, V>, BuildHasherDefault<NoHashHasher<HashedKey>>, Hybrid>>,
>;

// FlatMap in DRAM (alternative to DashMap)
#[cfg(feature = "global_flatmap_dram")]
pub type ObjectMapRef<K, V> =
    Arc<RwLock<crate::flatmap::FlatMapWithHasher<HashedKey, Object<K, V>, NoHasher>>>;

// FlatMap in PMEM (alternative to HashMap with Hybrid allocator)
#[cfg(feature = "global_flatmap_pmem")]
pub type ObjectMapRef<K, V> =
    Arc<RwLock<crate::flatmap::FlatMapWithHasher<HashedKey, Object<K, V>, NoHasher, Hybrid>>>;

// Hashbrown HashMap in DRAM (for performance comparison with global_hashtable_pmem)
#[cfg(feature = "hashbrown_dram")]
pub type ObjectMapRef<K, V> =
    Arc<RwLock<HashMap<HashedKey, Object<K, V>, BuildHasherDefault<NoHashHasher<HashedKey>>>>>;
...
fn with_object_ref<K, V, R>(
    objects: &ObjectMapRef<K, V>,
    hashed_key: &HashedKey,
    f: impl FnOnce(&Object<K, V>) -> R,
) -> Option<R> {
    objects.get(hashed_key).map(|object| f(&object))
}
...
fn with_object_ref<K, V, R>(
    ...
    let guard = objects.read().unwrap();
    guard.get(hashed_key).map(f)
}
...
fn with_object_mut<K, V, R>(
    objects: &ObjectMapRef<K, V>,
    hashed_key: &HashedKey,
    f: impl FnOnce(&mut Object<K, V>) -> R,
) -> Option<R> {
    objects
        .get_mut(hashed_key)
        .map(|mut object| f(&mut *object))
}
...
fn with_object_mut<K, V, R>(
    ...
    let mut guard = objects.write().unwrap();
    guard.get_mut(hashed_key).map(f)
}
...
fn insert_object<K, V>(
    objects: &ObjectMapRef<K, V>,
    hashed_key: HashedKey,
    object: Object<K, V>,
) -> Option<Object<K, V>> {
    objects.insert(hashed_key, object)
}
...
fn insert_object<K, V>(
    ...
    objects.write().unwrap().insert(hashed_key, object)
}
...
fn clear_objects<K, V>(objects: &ObjectMapRef<K, V>) {
    objects.clear();
}
...
fn create_object_map<K, V>(max_size: CacheSize) -> ObjectMapRef<K, V> {
    let _ = max_size;
    #[cfg(all(
        not(feature = "global_hashtable_pmem"),
        not(feature = "global_flatmap_dram"),
    ...
    {
        return Arc::new(DashMap::with_hasher(NoHasher::default()));
    }
    ...
    {
        return Arc::new(RwLock::new(HashMap::with_hasher_in(
            NoHasher::default(),
            Hybrid,
        )));
    }
    ...
    #[cfg(feature = "hashbrown_dram")]
    {
        return Arc::new(RwLock::new(HashMap::with_hasher(NoHasher::default())));
    }
    ...
    #[cfg(feature = "global_flatmap_dram")]
    {
        ...
        return Arc::new(RwLock::new(
            crate::flatmap::FlatMapWithHasher::with_capacity_and_hasher_unchecked(
                capacity,
                NoHasher::default(),
            ),
        ));
    ...
    #[cfg(feature = "global_flatmap_pmem")]
    {
        ...
        let capacity = (estimated_objects * 2).next_power_of_two();
        return Arc::new(RwLock::new(
            crate::flatmap::FlatMapWithHasher::with_capacity_hasher_in(
                capacity,
                NoHasher::default(),
                Hybrid,
            ),
...
pub struct PaperCache<K, V, S = RandomState> {
    ...
    #[cfg(all(
        feature = "key_value_pmem",
        any(feature = "enable_tiering_manager", feature = "sets_dram")
    ))]
...
where
    ...
    V: 'static + TypeSize + Clone + Send + Sync + AsRef<[u8]> + From<Vec<u8>>,
    S: Default + Clone + BuildHasher,
{
    ...
    pub fn with_hasher(
        ...
        let tiering_manager = {
            ...
            Arc::new_cyclic(|weak_tm: &std::sync::Weak<TieringManager<K, V>>| {
                ...
                let batch_persist_cb =
                    move |batch: Vec<crate::tiering::manager::PmemBackfillJob<K>>| {
                        ...
                        for job in batch {
                            ...
                            let mut pmem_vec =
                                Vec::<u8, Hybrid>::with_capacity_in(job.value.len(), Hybrid);
    ...
    pub fn get(&self, key: &K) -> Result<V, CacheError> {
        ...
        #[cfg(all(
            feature = "key_value_pmem",
            feature = "enable_tiering_manager",
            not(feature = "hashtable_tiering")
        ))]
        if let Some(dram_object) = self.tiering_manager.get_from_dram(&hashed_key) {
            if !dram_object.is_expired() && dram_object.key_matches(key) {
                self.status.incr_hits();
                self.broadcast(WorkerEvent::Get(hashed_key, true))?;
                return Ok(dram_object.data().as_ref().clone());
            }
        }
        ...
        #[cfg(all(
            feature = "key_value_pmem",
            feature = "enable_tiering_manager",
            feature = "hashtable_tiering"
        ))]
        if let Some(dram_object) = self.tiering_manager.get_from_dram(&hashed_key) {
            if !dram_object.is_expired() && dram_object.key_matches(key) {
                self.status.incr_hits();
                self.broadcast(WorkerEvent::Get(hashed_key, true))?;
                return Ok(V::from(dram_object.data_as_bytes()));
            }
        }

        let value = with_object_ref(&self.objects, &hashed_key, |object| {
        ...
        let result = if let Some(arc_val) = value {
            self.status.incr_hits();
    ...
    /// ).unwrap();
    ///
    /// assert!(cache.set(0, vec![0u8], None).is_ok());
    /// ```

    pub fn set(&self, key: K, value: impl Into<V>, ttl: Option<u32>) -> Result<(), CacheError> {
        let hashed_key = self.hash_key(&key);
        ...
        #[cfg(feature = "sets_dram")]
        {
            self.tiering_manager
                .set_dram(hashed_key, key.clone(), value.as_ref(), ttl);
            return Ok(());
        }
        ...
        let old_object_info = insert_object(&self.objects, hashed_key, object).map(|old_object| {
    ...
    pub fn has(&self, key: &K) -> bool {
        ...
        with_object_ref(&self.objects, &hashed_key, |object| {
            object.key_matches(key) && !object.is_expired()
        })
        .unwrap_or(false)
    }
    ...
    /// ).unwrap();
    ///
    /// cache.set(0, vec![0u8], None); // value will not expire
    /// cache.ttl(&0, Some(5)); // value will expire in 5 seconds
    /// ```
    pub fn ttl(&self, key: &K, ttl: Option<u32>) -> Result<(), CacheError> {
        let hashed_key = self.hash_key(key);

        let Some(result) = with_object_mut(&self.objects, &hashed_key, |object| {
            if object.key_matches(key) && !object.is_expired() {
                ...
                Some((old_expiry, old_base_size, new_expiry, new_base_size))
            } else {
                None
            }
        })
        ...
        let (old_expiry, old_base_size, new_expiry, new_base_size) = result;
...
#[cfg(all(
    any(feature = "global_hashtable_pmem", feature = "hashbrown_dram"),
    not(feature = "global_flatmap_dram"),
    not(feature = "global_flatmap_pmem")
))]
pub fn erase<K, V>(
    ...
    V: TypeSize,
{
    ...
    let object = objects_lock
        .remove_unchecked(&hashed_key)
        .ok_or(CacheError::KeyNotFound)?;
    let base_size = overhead_manager.base_size(&object) as i64;
...
mod tests {
    ...
    fn it_does_not_allow_empty_policies() {
        let try_cache =
            PaperCache::<u32, BufferDRAM>::new(TEST_CACHE_MAX_SIZE, &[], PaperPolicy::Lfu);

        assert!(try_cache.is_err_and(|err| err == CacheError::EmptyPolicies));

        let try_cache =
            PaperCache::<u32, BufferDRAM>::new(TEST_CACHE_MAX_SIZE, &[], PaperPolicy::Auto);
    ...
    fn it_does_not_allow_auto_policy_in_configured_policies() {
        let try_cache = PaperCache::<u32, BufferDRAM>::new(
            TEST_CACHE_MAX_SIZE,
            &[PaperPolicy::Auto],
            PaperPolicy::Auto,
        );
...
mod test_global_hashtable_pmem_alone {
    ...
    fn test_basic_operations() {
        ...
        let cache: PaperCache<u32, Box<[u8]>, RandomState> =
            PaperCache::new(1000000, &[PaperPolicy::Lfu], PaperPolicy::Lfu)
    ...
    fn test_multiple_keys() {
        let cache: PaperCache<u32, Box<[u8]>, RandomState> =
            PaperCache::new(10000000, &[PaperPolicy::Lru], PaperPolicy::Lru)
                .expect("Failed to create cache");
    ...
    #[test]
    fn test_wipe() {
        let cache: PaperCache<String, Box<[u8]>, RandomState> =
            PaperCache::new(1000000, &[PaperPolicy::Lfu], PaperPolicy::Lfu)
                .expect("Failed to create cache");
...
mod test_new_features {
    ...
    fn test_cache_init_with_lfu_eviction() {
        let cache: PaperCache<u32, Box<[u8]>, RandomState> =
            PaperCache::new(1_000_000, &[PaperPolicy::Lfu], PaperPolicy::Lfu)
...
mod test_hw_perf {
    ...
    fn test_hw_perf_measure_operation() {
        ...
        assert_eq!(
            result, 499500u64,
            "operation result must be correct regardless of perf counter availability"
        );
    }
}
