...
//! the object-map row, because the row's content drops from 32 bytes to 16 and
//! the measured allocation follows: 80 -> 40 B/object, value-size independent.
//!
//! It loses on two counts, and both are intrinsic rather than tunable.
//!
//! JEMALLOC SIZE CLASSES. Measured with `measure_object_map_point`, one
//! process per point, n = 2^20, against this two-allocation shape:
//!
//! ```text
//!   value     2 allocs    1 alloc     delta
//!       64          144        136        -8
//!      100          192        168       -24
//!      256          336        360       +24
//!     1000         1104       1064       -40
...
//! ```
//!
//! `nallocx(4096)` is 4096 but `nallocx(24 + 4096)` is 5120, so ANY header in
//! front of a class-aligned value costs a whole class. A suffix is identical --
//! only the total reaches `nallocx` -- and shrinking the header does not help,
//! because the strong count has to be in the allocation for the last handle to
//! find it: `nallocx(4 + 4096)` is 5120 too. Powers of two are common in both
//! benchmarks and real workloads.
//!
//! THE READ PATH. Co-locating puts the count and the metadata in the object's
//! own tier, so a `get` that hits a slow-tier object pays, before it copies a
//! single byte: one remote cache line for `key_matches` and `is_expired`, and
//! TWO remote atomic read-modify-writes for the refcount bump and drop.
//! Atomics do not pipeline and cannot be prefetched, so they serialise where a
//! streaming copy would not. A DRAM header makes all three local and leaves
//! only the payload remote -- and it also sends no remote traffic at all for
//! an object it is about to reject as expired.
//!
//! Two things worth keeping from the experiment:
//!
//!   * PREFETCH THE PAYLOAD BEFORE THE CHECKS. Nothing here does. It hides the
//!     dependent load that gates every slow-tier read, works for either header
//!     placement, and wastes nothing on the reject path.
//!   * THE KEY IS ONLY INLINE FOR POD KEYS. `K` is stored as a struct, so a
//!     `String` or `Vec<u8>` key keeps its bytes in a separate GLOBAL
//!     allocation -- always node 0, never tiered, and a second pointer chase
//!     for every collision check. Inlining key BYTES as a second tail would fix
//!     it and needs a bytes-oriented bound on `K`, which the generic API
