...
//! Integration tests for the `s3_fifo_compact_hybrid_cache` feature.
//!
//! A deliberate near-copy of the baseline suite. This stack is a compaction of
//! that one and must be behaviourally indistinguishable from it, so it has to
//! answer the same behavioural questions -- not a reduced set chosen by whoever
//! wrote the new stack.
//!
//! Unit tests cannot substitute. They drive the stack directly and never place
//! any bytes, so they cannot see a placement bug: on the LFU conversion,
//! `hybrid_policy::admission_tier` had no arm for the new policy and every
//! brand-new object was built in DRAM while the stack recorded it as slow. The
//! unit tests were green throughout. Only a suite that runs a real cache sees
//! that class of defect.
//!
//! Run with nightly (required for `allocator_api` via `key_value_pmem`):
//!   cargo +nightly test --test s3_fifo_compact_hybrid_cache_integration --features s3_fifo_compact_hybrid_cache
//!
...
#[cfg(feature = "s3_fifo_compact_hybrid_cache")]
mod hybrid_cache_tests {
    ...
    fn ensure_pmem_allocator_warm() {
        ...
        let cache = PaperCache::<u32, TieredBuffer>::new(1_048_576, CacheTierSize::Bytes(1_048_576), PaperPolicy::S3FifoCompactHybrid(SLACK_RATIO))
    ...
    fn admission_always_lands_in_slow_tier() {
        ...
        let cache = PaperCache::<u32, TieredBuffer>::new(
            1_048_576,
            CacheTierSize::Bytes(1_048_576), PaperPolicy::S3FifoCompactHybrid(SLACK_RATIO)).expect("cache should construct");
    ...
    fn one_access_key_aging_out_without_reaccess_is_evicted() {
        ...
        let cache = PaperCache::<u32, TieredBuffer>::new(
            256,
            CacheTierSize::Bytes(256), PaperPolicy::S3FifoCompactHybrid(0.9)).expect("cache should construct");
    ...
    fn one_access_capacity_pressure_evicts_before_global_max_size_is_reached() {
        ...
        let cache = PaperCache::<u32, TieredBuffer>::new(
            ...
            CacheTierSize::Bytes(1_048_576), PaperPolicy::S3FifoCompactHybrid(0.00002)).expect("cache should construct");
    ...
    fn fast_tier_pressure_within_main_queue_demotes_the_oldest() {
        ...
        let cache = PaperCache::<u32, TieredBuffer>::new(
            ...
            CacheTierSize::Bytes(1_600), PaperPolicy::S3FifoCompactHybrid(SLACK_RATIO)).expect("cache should construct");
    ...
    fn ttl_survives_a_demotion() {
        ...
        let cache = PaperCache::<u32, TieredBuffer>::new(
            1_048_576,
            CacheTierSize::Bytes(TTL_FAST_TIER), PaperPolicy::S3FifoCompactHybrid(SLACK_RATIO)).expect("cache should construct");
    ...
    fn terminal_eviction_prefers_one_access_queue_over_main_queue() {
        ...
        let cache = PaperCache::<u32, TieredBuffer>::new(
            256,
            CacheTierSize::Bytes(256), PaperPolicy::S3FifoCompactHybrid(0.5)).expect("cache should construct");
    ...
    fn resize_rescales_one_access_capacity() {
        ...
        let cache = PaperCache::<u32, TieredBuffer>::new(1_024, CacheTierSize::Bytes(1_024), PaperPolicy::S3FifoCompactHybrid(0.5))
    ...
    fn zero_fast_tier_size_is_rejected() {
        let result = PaperCache::<u32, TieredBuffer>::new(1_024, CacheTierSize::Bytes(0), PaperPolicy::S3FifoCompactHybrid(0.5));
        assert!(matches!(result, Err(CacheError::InvalidFastTierSize)));
    }

    #[test]
    fn fast_tier_size_exceeding_max_size_is_rejected() {
        let result = PaperCache::<u32, TieredBuffer>::new(1_024, CacheTierSize::Bytes(2_048), PaperPolicy::S3FifoCompactHybrid(0.5));
        assert!(matches!(result, Err(CacheError::InvalidFastTierSize)));
    }

    #[test]
    fn zero_max_size_is_rejected() {
        let result = PaperCache::<u32, TieredBuffer>::new(0, CacheTierSize::Bytes(100), PaperPolicy::S3FifoCompactHybrid(0.5));
    ...
    fn a_zero_length_one_access_queue_is_accepted() {
        ...
        assert!(
            PaperCache::<u32, TieredBuffer>::new(1_024, CacheTierSize::Bytes(512), PaperPolicy::S3FifoCompactHybrid(0.0))
                .is_ok(),
        );
        ...
        assert!(
            PaperCache::<u32, TieredBuffer>::new(1_024, CacheTierSize::Bytes(512), PaperPolicy::S3FifoCompactHybrid(0.0005))
                .is_ok(),
        );
    }
    ...
    fn an_extreme_ratio_is_accepted_when_max_size_leaves_both_queues_a_byte() {
        assert!(
            PaperCache::<u32, TieredBuffer>::new(
                ...
                PaperPolicy::S3FifoCompactHybrid(0.9995),
            )
            .is_ok(),
        );
    }
    ...
    fn invalid_one_access_ratio_is_rejected() {
        assert!(matches!(
            PaperCache::<u32, TieredBuffer>::new(1_024, CacheTierSize::Bytes(512), PaperPolicy::S3FifoCompactHybrid(1.5)),
            Err(CacheError::InvalidPolicy),
        ));

        assert!(matches!(
            PaperCache::<u32, TieredBuffer>::new(1_024, CacheTierSize::Bytes(512), PaperPolicy::S3FifoCompactHybrid(-0.1)),
            Err(CacheError::InvalidPolicy),
        ));
        ...
        assert!(matches!(
            PaperCache::<u32, TieredBuffer>::new(1_024, CacheTierSize::Bytes(512), PaperPolicy::S3FifoCompactHybrid(1.0)),
            Err(CacheError::InvalidPolicy),
        ));
        ...
        assert!(matches!(
            PaperCache::<u32, TieredBuffer>::new(1_024, CacheTierSize::Bytes(512), PaperPolicy::S3FifoCompactHybrid(0.9995)),
            Err(CacheError::InvalidPolicy),
        ));
    }
    ...
    fn tiny_fast_tier_still_allows_one_access_admission() {
        ...
        let cache = PaperCache::<u32, TieredBuffer>::new(
            1_048_576,
            CacheTierSize::Bytes(1), PaperPolicy::S3FifoCompactHybrid(SLACK_RATIO)).expect("cache should construct");
