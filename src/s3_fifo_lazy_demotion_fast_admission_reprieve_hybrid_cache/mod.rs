...
//! evicted. See
//! `worker::policy::policy_stack::s3_fifo_lazy_demotion_fast_admission_reprieve_hybrid_stack`'s
...
pub use stats::S3FifoLazyDemotionFastAdmissionReprieveHybridStats;

/// Marker type selecting
/// `s3_fifo_lazy_demotion_fast_admission_reprieve_hybrid_cache`'s
...
pub struct S3FifoLazyDemotionFastAdmissionReprieveHybridPolicy;

impl crate::hybrid_policy::HybridPolicy for S3FifoLazyDemotionFastAdmissionReprieveHybridPolicy {
	type Stats = S3FifoLazyDemotionFastAdmissionReprieveHybridStats;
	type ExtraConfig = f64;

	fn seed_policy(ratio: f64) -> crate::PaperPolicy {
		crate::PaperPolicy::S3FifoLazyDemotionFastAdmissionReprieveHybrid(ratio)
	}

	fn stats_from_status(status: &crate::status::AtomicStatus) -> S3FifoLazyDemotionFastAdmissionReprieveHybridStats {
		status.s3_fifo_lazy_demotion_fast_admission_reprieve_hybrid_stats()
	}
