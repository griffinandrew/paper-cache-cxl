//! ONE ALLOCATION PER VALUE -- selected by the `fused_value` feature.
//!
//! The alternative layout lives in `value.rs` and is the default. Read that
//! module's "Why the bytes are a SECOND allocation" section before choosing
//! between them: this one was built, measured and reverted, and the two
//! numbers that decided it are jemalloc size classes (any header in front of a
//! class-aligned value costs a whole class -- up to +2008 B/object at 8 KiB)
//! and the slow-tier read path (the count and metadata move into the object's
//! own tier, so a get on a slow object pays a remote cache line and two remote
//! atomic read-modify-writes before it copies a byte).
//!
//! What it buys: an object is one allocation on one tier, so the metadata
//! tiers WITH the bytes instead of being pinned to DRAM.
//!
/*
...
mod tests {
	...
	fn routing_lock() -> MutexGuard<'static, ()> {
		...
		// Forward-ported from 1daa093, which landed after this layout was
		// reverted and so never reached this copy.
	...
	/// Where the METADATA lands -- the question the test above does not ask.
	///
	/// `values_land_on_the_numa_node_their_tier_names` checks
	...
	#[test]
	#[cfg(not(feature = "stock_jemalloc"))]
	fn a_slow_value_puts_its_metadata_on_the_slow_node_too() {
		use crate::numa_alloc::{self, tests::node_of, NODE_SLOW};

		assert!(numa_alloc::init(), "the node-0 and node-1 arena pools must build");

		let _guard = routing_lock();

		// Page-sized for the same reason the sibling test is: `get_mempolicy`
		// answers for a PAGE, and an untouched page has no node.
		let bytes = vec![0x5Au8; 8192];
		let slow = TieredValue::new_slow(KEY, &bytes, None);

		let header_node = node_of(slow.raw_ptr() as *const u8);
		let bytes_node = node_of(slow.bytes().as_ptr());

		assert_eq!(
			bytes_node, NODE_SLOW as i32,
			"a slow value's BYTES must be on node {NODE_SLOW}, not {bytes_node}",
		);

		// The discriminating half, and the property this layout exists for:
		// the count, key, length and expiry sit at a fixed offset INSIDE the
		// block the slow-tier allocator returned, so a demotion moves them
		// with the payload rather than leaving them behind in DRAM.
		assert_eq!(
			header_node, NODE_SLOW as i32,
			"the metadata must travel with the bytes: expected node {NODE_SLOW}, \
			 got {header_node}",
		);
		assert_eq!(
			header_node, bytes_node,
			"one allocation means one node -- header {header_node}, bytes {bytes_node}",
		);
	}
