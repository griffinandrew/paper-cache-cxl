...
mod tests {
    ...
    fn ghost_queue_routes_reinserted_key_to_main() {
        ...
        assert_eq!(
            evicted,
            Some(0),
            "tail of small queue (key 0, freq=0) evicted first"
        );
        ...
        assert_eq!(
            stack.evict_one(),
            None,
            "stack must be empty after all evictions"
        );
    }
    ...
    fn no_ghost_entry_routes_fresh_insertion_to_small_queue() {
        ...
        assert_eq!(
            evicted,
            Some(0),
            "tail of small queue (key 0, freq=0) evicted into ghost"
        );
        assert!(
            !stack.contains(0),
            "key 0 must be absent from active queues"
        );
        ...
        assert!(
            stack.contains(2),
            "freshly inserted key 2 must be in the stack"
        );
        ...
        assert_eq!(
            stack.evict_one(),
            None,
            "stack must be empty after draining"
        );
    }
}
