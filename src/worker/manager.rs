...
use crate::{
    ObjectMapRef, OverheadManagerRef, StatusRef,
    error::CacheError,
...
use crate::{tiering::TieringManager, worker::TieringWorker};
...
impl WorkerManager {
        ...
        V: 'static + TypeSize + Clone + AsRef<[u8]>,
    {
        ...
        let manager = WorkerManager { listener, workers };
        ...
        V: 'static + TypeSize + Clone + AsRef<[u8]>,
    {
        ...
        let workers: Arc<Box<[WorkerSender]>> =
            Arc::new(Box::new([policy_worker, ttl_worker, tiering_worker]));
    ...
    pub fn new_with_eviction_callback<K, V>(
        ...
        status: &StatusRef,
        overhead_manager: &OverheadManagerRef,
        eviction_callback: Box<
            dyn for<'a> Fn(crate::HashedKey, std::sync::Arc<V>, &'a K) + Send + Sync,
        >,
