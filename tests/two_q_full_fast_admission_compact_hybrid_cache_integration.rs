...
//! Integration tests for the `two_q_full_fast_admission_compact_hybrid_cache` feature.
//!
...
//!   cargo +nightly test --test two_q_full_fast_admission_compact_hybrid_cache_integration --features two_q_full_fast_admission_compact_hybrid_cache
//!
...
mod hybrid_cache_tests {
    ...
    fn ensure_pmem_allocator_warm() {
        ...
        let cache = PaperCache::<u32, TieredBuffer>::new(
            ...
            PaperPolicy::TwoQFullFastAdmissionCompactHybrid(0.0005, 0.5),
    ...
    fn admission_lands_in_a1_in_in_the_fast_tier_immediately() {
        ...
        let cache = PaperCache::<u32, TieredBuffer>::new(
            ...
            PaperPolicy::TwoQFullFastAdmissionCompactHybrid(0.1, 0.2),
        ).expect("cache should construct");
    ...
    fn ageing_out_of_a1_in_demotes_into_a1_out_rather_than_evicting() {
        ...
        let cache = PaperCache::<u32, TieredBuffer>::new(
            ...
            PaperPolicy::TwoQFullFastAdmissionCompactHybrid(0.001, 0.5),
        ).expect("cache should construct");
    ...
    fn a1_out_capacity_pressure_evicts_without_leaking_the_object() {
        ...
        let cache = PaperCache::<u32, TieredBuffer>::new(
            1_048_576,
            CacheTierSize::Bytes(524_288),
            PaperPolicy::TwoQFullFastAdmissionCompactHybrid(0.0002, 0.0002),
        ).expect("cache should construct");
    ...
    fn eviction_prefers_a1_out_over_the_main_queue() {
        ...
        let cache = PaperCache::<u32, TieredBuffer>::new(
            1_048_576,
            CacheTierSize::Bytes(524_288),
            PaperPolicy::TwoQFullFastAdmissionCompactHybrid(0.0002, 0.0005),
        ).expect("cache should construct");
    ...
    fn invalid_construction_parameters_are_rejected() {
        assert!(matches!(
            PaperCache::<u32, TieredBuffer>::new(1_048_576, CacheTierSize::Bytes(0), PaperPolicy::TwoQFullFastAdmissionCompactHybrid(0.1, 0.2)),
            Err(CacheError::InvalidFastTierSize),
        ));

        assert!(matches!(
            PaperCache::<u32, TieredBuffer>::new(1_024, CacheTierSize::Bytes(2_048), PaperPolicy::TwoQFullFastAdmissionCompactHybrid(0.1, 0.2)),
            Err(CacheError::InvalidFastTierSize),
        ));

        assert!(matches!(
            PaperCache::<u32, TieredBuffer>::new(0, CacheTierSize::Bytes(1), PaperPolicy::TwoQFullFastAdmissionCompactHybrid(0.1, 0.2)),
            Err(CacheError::ZeroCacheSize),
        ));

        // k_in out of range.
        assert!(matches!(
            PaperCache::<u32, TieredBuffer>::new(1_048_576, CacheTierSize::Bytes(1_024), PaperPolicy::TwoQFullFastAdmissionCompactHybrid(1.5, 0.2)),
            Err(CacheError::InvalidPolicy),
        ));

        assert!(matches!(
            PaperCache::<u32, TieredBuffer>::new(1_048_576, CacheTierSize::Bytes(1_024), PaperPolicy::TwoQFullFastAdmissionCompactHybrid(-0.1, 0.2)),
            Err(CacheError::InvalidPolicy),
        ));

        // k_out out of range -- the half a single-ratio check would miss.
        assert!(matches!(
            PaperCache::<u32, TieredBuffer>::new(1_048_576, CacheTierSize::Bytes(1_024), PaperPolicy::TwoQFullFastAdmissionCompactHybrid(0.1, 1.5)),
            Err(CacheError::InvalidPolicy),
        ));

        assert!(matches!(
            PaperCache::<u32, TieredBuffer>::new(1_048_576, CacheTierSize::Bytes(1_024), PaperPolicy::TwoQFullFastAdmissionCompactHybrid(0.1, -0.1)),
            Err(CacheError::InvalidPolicy),
        ));
    }
