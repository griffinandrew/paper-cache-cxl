...
use crate::{
    CacheSize, EraseKey, HashedKey, ObjectMapRef, OverheadManagerRef, StatusRef, erase,
    error::CacheError,
    object::ObjectSize,
    policy::PaperPolicy,
    worker::{
        Worker, WorkerEvent, WorkerReceiver, WorkerSender,
        policy::{
    ...
    V: TypeSize,
{
    fn run(&mut self) -> Result<(), CacheError> {
        ...
        loop {
            let events = self.listener.try_iter().collect::<Vec<WorkerEvent>>();
    ...
    V: TypeSize,
{
    ...
    fn handle_get(&mut self, key: HashedKey, hit: bool) {
        if let Some(stack) = &mut self.policy_stack {
            if let AccessOutcome::GhostHit = stack.record_access(key, hit) {
                debug_assert!(
                    self.promotion_tx.is_some(),
                    "promotion channel must exist for ghost hits"
                );
    ...
    fn apply_evictions(&mut self, buffered_events: &mut Vec<StackEvent>) -> Result<(), CacheError> {
        ...
        while self.status.used_size(&policy) > max_cache_size {
            ...
            let maybe_key = policy_stack.evict_one().map(|key| EraseKey::Hashed(key));
